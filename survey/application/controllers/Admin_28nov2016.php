...
class Admin extends CI_Controller 
{
    ...
    public function ManageAdmin()
	{
        ...
        for ($_i = 0; $_i < $resultSize2; ++$_i) 
		{
            ...
            $appObject[$_i]['AdminName'] =$appObject[$_i]['FirstName']." ".$appObject[$_i]['LastName'];
            if($appObject[$_i]['IsVerify']==1){
                $veify='Verified';
            }else{
                $veify='<a href="' . base_url() . 'Admin/VerifyAdmin/User_Id/' . $appObject[$_i]['Admin_Id'] . '" title="' . $appObject[$_i]['FirstName'] . '">Verify Admin</a>';
            }
            $linkval=$veify.' | <a href="' . base_url() . 'Admin/EditAdmin/User_Id/' . $appObject[$_i]['Admin_Id'] . '" title="' . $appObject[$_i]['FirstName'] . '">Edit</a> |';
            $linkval.="<a href='".base_url()."Admin/DeleteAdmin/User_Id/".$appObject[$_i]['Admin_Id']."/".$appObject[$_i]['FirstName']."' title='".$appObject[$_i]['AdminName']."' onclick='return confirm(\"Are you sure to want delete this record?\");' >Delete</a>";
 
            $appObject[$_i]['Action'] =$linkval;
            unset($appObject[$_i]['FirstName']);
            unset($appObject[$_i]['LastName']);
    ...
    public function ManageSurveyUser() 
	{
           ...
           // $this->Adminmodel->delDataAll('usersurvey', 'SureveyId', $survey_Id); // first delete then insert data
            ...
            if(count($surveyuserlist)>0)
			{
                foreach($surveyuserlist as $ulist){
                    $this->Surevey_Id = $survey_Id;
                    $this->User_Id = $ulist;
                    $this->CreatedBy = $admin_id;
                    $this->CreatedDate = date('Y-m-d H:i:s');
                    $this->UpdatedBy = $admin_id;
                    $this->UpdatedDate = date('Y-m-d H:i:s');
					$checkUserSurvey=$this->Adminmodel->getCommonData('UserSurvey', array('User_Id'=>$ulist,'Surevey_Id'=>$survey_Id));
					if(count($checkUserSurvey)==0)
					{
						$insert = $this->Adminmodel->insertEntry('UserSurvey', $this);	
					}
					/*Here Insert data in SurveyGroupUsers */
					$checkUserSurveyU=$this->Adminmodel->getCommonData('SurveyGroupUsers', array('User_Id'=>$ulist,'SurveyGroup_Id'=>$SurveyGroupId));
					if(count($checkUserSurveyU)==0)
					{
						$getUserDetails=$this->Adminmodel->getData('UserMaster', 'User_Id', $ulist);
						$email=$getUserDetails[0]['Email'];
						$username=$getUserDetails[0]['FirstName']." ".$getUserDetails[0]['LastName'];
						
						$usergarray=array('User_Id'=>$ulist, 'UserName'=>$username, 'Email'=>$email, 'LibraryName'=>$LibraryName, 'SurveyGroup_Id'=>$SurveyGroupId, 'CreatedBy'=>$admin_id, 'CreatedDate'=>date('Y-m-d H:i:s'), 'UpdatedBy'=>$admin_id, 'UpdatedDate'=>date('Y-m-d H:i:s'));
						$insertGropuser = $this->Adminmodel->insertEntry('SurveyGroupUsers', $usergarray);
					}
					/*End Here*/					
                }
                /*Remove left user*/
				...
				if(count($diffarray)>0)
				{
					foreach($diffarray as $left){
						$deletearray=array('User_Id'=>$left,'SurveyGroup_Id'=>$SurveyGroupId);
						$this->Adminmodel->deleteDataAllCOM('SurveyGroupUsers',$deletearray);
						
						$deletearray=array('User_Id'=>$left,'Surevey_Id'=>$survey_Id);
						$this->Adminmodel->deleteDataAllCOM('UserSurvey',$deletearray);
					}
				}
                /*End Here*/
        ...
        $getUserRec = $this->Adminmodel->getData('UserSurvey', 'Surevey_Id', $survey_Id);
        $selecteduser=array();
        foreach($getUserRec as $selulist){
            $selecteduser[]=$selulist['User_Id'];
        }
    ...
    public function ManageSurveyUrl()
	{
       ...
       // echopre($getUserRec);
        if ($this->input->post('submit')) 
		{
            ...
            $admin_id = $_SESSION['Admin_Id'];
            $deletearray=array('Surevey_Id'=>$survey_Id,'Round'=>$roundid);
            $this->Adminmodel->deleteDataAllCOM('UserSurveyRound', $deletearray);
            ...
            if(count($surveyuserlist)>0)
			{
				...
				{
                    $arraylist=array('Surevey_Id'=>$survey_Id,'Round'=>$roundid,'User_Id'=>$ulist);
                    ...
                    //if(count($checkuserRoundassign)==0){
                        ...
                        $this->UpdatedBy = $admin_id;
                        $this->UpdatedDate = date('Y-m-d H:i:s');
                        $insert = $this->Adminmodel->insertEntry('UserSurveyRound', $this);
                    //}
					
					/*Here insert into SurveyIteration */
					$getSurveyGroupUser_Id=$this->Adminmodel->getCommonData('SurveyGroupUsers', array('SurveyGroup_Id'=>$SurveyGroupId,'User_Id'=>$ulist));
					$SurveyGroupUser_Id=$getSurveyGroupUser_Id[0]['SurveyGroupUser_Id'];
					$checkexitassignURL=$this->Adminmodel->getCommonData('SurveyIteration', array('SurveyGroupUser_Id'=>$SurveyGroupUser_Id,'Iteration'=>$roundid));
        ...
        if ($this->input->post('sendurlmail')) 
		{
            ...
            if(count($surveyuserlist)>0)
			{
                foreach($surveyuserlist as $key=>$ulist)
				{
                    $row_userSurveyURL=$this->Adminmodel->getSurveyURL($survey_Id, $roundid, $ulist);
					if($row_userSurveyURL)
					{
						$url=$row_userSurveyURL->SurveyURL;
						$url='<a href="'.BASE_URL_SURVEY.$url.'" target="_blank">'.BASE_URL_SURVEY.$url.'</a>';
						
						$userdata=$this->Adminmodel->getData('UserMaster','User_Id',$ulist);
						$fname=$userdata[0]['FirstName'];
						$lname=$userdata[0]['LastName'];
						$usermail=$userdata[0]['Email'];

						$templatedata=$this->Adminmodel->getCommonData('TemplateMaster',array('TemplateName'=>'Email','CreatedBy'=>$admin_id));
						...
						$this->Adminmodel->updateSendEmailSurveyIteration($survey_Id, $roundid, $ulist);
					}
                }
        ...
        foreach($getUserRecSel as $selulist){
            $selecteduser[$selulist['User_Id']]=$selulist['SurveyURL'];
			if($selulist['SendEmail'] == '1')
			{
				$mailSended[$selulist['User_Id']]='Yes';
			}
			else
			{
				$mailSended[$selulist['User_Id']]='No';
			}
			
			// $selecteduser[$selulist['User_Id']]=$selulist['SendEmail'];
		}
