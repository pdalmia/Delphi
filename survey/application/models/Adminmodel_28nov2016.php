...
class Adminmodel extends CI_Model 
{
	...
	{
        $this->db->select('AM.Admin_Id,FirstName,LastName,Company,Email,Phone,IsVerify');
	...
	{
		$admin_id = $_SESSION['Admin_Id'];
        $this->db->select('TM.Template_Id,TM.TemplateName');
    ...
    function getDataUserassignsurvey($Survey_Id)
	{
        ...
        $this->db->where('a.Surevey_Id', $Survey_Id);
    ...
    function getDataUserassignsurveyURL($SurveyGroup_Id,$Iteration)
	{
	...
	{
		$this->db->select('SGU.UserName,SI.SurveyURL,SIA.SurveyQuestion_Id,SIA.SurveyProposition_Id,SIA.Rating,SIA.Reason,SIA.CreatedDate,SIA.UpdatedDate');
    ...
    function deleteMultipleUser($userArray)
    {
        ...
        $sql = "DELETE FROM UserSurveyRound where User_Id not in (".$impldestr.")";
        $this->db->query($sql);
        return true;
    }
    ...
    function getSurveyGroupQuestion($SurveyId,$SurveyQuestionGroup_Id)
	{
        ...
        $this->db->select('SQ.*');
        $this->db->from('SurveyQuestions SQ');
        $this->db->where('SQ.SurveyId', $SurveyId);
        $this->db->where('SQ.SurveyQuestionGroup_Id', $SurveyQuestionGroup_Id);
		$this->db->where('SQ.IsDelete', 0);
        $this->db->order_by('SQ.QuestionOrder', 'asc');
        $query=$this->db->get();
